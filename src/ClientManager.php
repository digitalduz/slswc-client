...
class ClientManager {
	...
	public static function list_products( $products ) {
		...
		Logger::log( 'Local products.' );
		Logger::log( $products );
		Logger::log( 'Remote Products' );
		Logger::log( $remote_products );
	...
	public static function get_remote_product( $slug = '', $type = 'plugin' ) {
		...
		Logger::log( 'Get remote product' );
		Logger::log( $response->product );
	...
	public static function get_remote_products( $type = 'plugins', $args = array() ) {
		...
		Logger::log( 'Getting remote products' );
		Logger::log( $response );
	...
	public static function server_request( $action = 'check_update', $request_info = array(), $domain = '' ) {
		...
		if ( ! is_wp_error( $result ) ) {
			...
			Logger::log( 'There was an error executing this request, please check the errors below.' );
			// phpcs:disable
