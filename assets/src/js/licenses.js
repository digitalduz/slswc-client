jQuery(function ($) {
  ...
  const slswcClient = {
    ...
    processAction: function (e) {
      ...
      $(button).html(
        `<img src="${window.slswc_updater_licenses.loader_url}" width="12" height="12"/> Processing`
      );

      let slug = $(this).data('slug');

      let license_key = $('#' + slug + '_license_key').val();
      let version = $('#' + slug + '_current_version').val();

      //let license_key = $(this).data('license_key');
      //let version = $(this).data('version');

      let license_action = $(this).data('action');
      let nonce = $(this).data('nonce') ?? window.slswc_updater_licenses.nonce;
      let domain = $(this).data('domain') ?? $;
      let environment = '';

      if (
        $('#' + slug + '_force-client-environment').is(':checked') &&
        $('#' + slug + '_environment').is(':visible')
      ) {
      ...
      $.ajax({
        url: window.slswc_updater_licenses.ajax_url,
        data: {
        ...
        success: function (response) {
          ...
          switch (response.status) {
            case 'active':
              ...
              $(button).html(window.slswc_updater_licenses.text_deactivate);
              $(button).data('action', 'deactivate');
              break;
            default:
              slswcClient.notice(response.message, 'error');
              $(button).html(window.slswc_updater_licenses.text_activate);
